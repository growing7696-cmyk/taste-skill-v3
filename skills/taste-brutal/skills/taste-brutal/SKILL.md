---
name: design-taste-brutalist
description: Neo-brutalist frontend skill for landing pages, portfolios, and campaigns that need to look loud, raw, and deliberately unpolished. Forks the taste-skill engine (active Layout Casting, real block library, multi-variant mode, measured performance gate) but inverts the aesthetic: hard edges, thick black borders, flat fluorescent color, system and monospace type, brutal contrast. This is the anti-polish counterpart to the base skill, not a refinement of it. Use when the brief wants attitude, not elegance.
---
...
## 4.0 BRUTALIST AESTHETIC OVERRIDE (read this first, it inverts Section 4)

> **This section overrides the aesthetic rules that follow.** The rest of Section 4, plus Sections 7 and 9, were written for a polished, refined look. This skill is neo-brutalist: loud, raw, deliberately unpolished. Where a rule below conflicts with the polished defaults later in this document, THIS section wins. The engine (Layout Casting in 1.5, blocks, multi-variant in 1.6, the performance gate in 15) is unchanged; only what counts as good-looking is inverted.

**The one-line philosophy.** Refined design hides its construction. Brutalism shows it. Hard edges, exposed structure, flat color, system type, and contrast so high it is almost rude. It should look confident and intentional, never like a polished site that failed to load its styles. The line between brutalism and broken is *intent expressed through consistency*: every hard choice is applied everywhere, on purpose.

### 4.0.0 Pick a mode (this skill now covers both brutalisms)

This package absorbs what used to be a separate `industrial-brutalist-ui` skill. Two brutalist skills that never referenced each other produced exactly the conflict the core contract exists to prevent, so they are one skill with two committed modes.

- **MODE: neo** - loud, flat, fluorescent, poster-like. Thick black borders, hard offset shadows, saturated flat color, system and grotesk type. This is what the rest of Section 4.0 describes.
- **MODE: industrial** - Swiss print meets tactical telemetry. Rigid grids, extreme type-scale contrast, utilitarian two-color substrate, simulated analog degradation. Full spec in `reference/modes.md`.

**Pick one and commit.** Declare it in the output header: `STYLE: taste-brutal, MODE: neo`. Do not mix substrates, do not blend the type systems, do not alternate between modes inside one page.

### 4.0.0.A The accessibility carve-outs both modes obey (HARD)

Brutalism fails the accessibility floor in four predictable places. These are not negotiable, and every one of them can be fixed without softening the look.

- **Type size.** Body prose at 16px minimum. Monospace and micro labels at 14px minimum, never 10px. Wide-tracked uppercase at small sizes is the least readable combination available.
- **All caps.** Headlines, labels, and data of five words or fewer. Never a paragraph. An all-caps body block is a reading failure wearing a design decision.
- **Overlays.** Scanlines, halftone, grain, and CRT effects live on a `position: fixed; pointer-events: none` layer, are capped at 0.10 opacity over any region containing text, and their contrast is measured on the composited result, not on the text color alone. They are disabled under `prefers-contrast: more`, and any that move are disabled under `prefers-reduced-motion: reduce`.
- **Fluorescent pairs.** Electric yellow on white fails. Measure every accent against its actual substrate before shipping it. `npm run verify:a11y` is the check, not your eye.

Decorative ASCII framing, crosshairs, barcode rules, and glyph clusters carry `aria-hidden="true"`. Invented telemetry strings are fine as decoration and are never presented as live system data.

### 4.0.A Color (inverts 4.2, 9.A, 9.F)

- **Pure black `#000` and pure white `#fff` are REQUIRED, not banned.** The base skill forbids them to preserve depth; brutalism wants the flatness. Use them.
- **Flat fluorescent accents, used LOUD.** Electric yellow (`#e7fe55` / `#ccff00`), hot magenta (`#ff2d9b`), cyan (`#00e5ff`), red (`#ff3b30`). Fully saturated. The base skill caps saturation under 80%; here saturation is the point.
- **Multiple accents allowed.** The base skill's Color Consistency Lock (one accent per page) is LIFTED. A neo-brutalist page can carry two or three fighting fluorescents, as long as the set is chosen up front and repeated, not random.
- **No gradients, no glows, no tints.** Flat fills only. A color is one hex, edge to edge. The LILA rule still holds in spirit (no soft AI-purple mush) but for the opposite reason: not "too flashy" but "not flat enough".
- **Color blocking over subtlety.** Whole sections in a single fluorescent field with black type on top. This replaces the base skill's neutral-base-plus-one-accent model.

### 4.0.B Shape and material (inverts 4.4, the Shape Lock, 9.A)

- **border-radius: 0. Everywhere.** No rounded corners, no pills. The Shape Consistency Lock still applies, but the locked value is always sharp.
- **Thick hard borders.** `2px`-`4px` solid black on cards, buttons, inputs, images. Borders are the primary structural device, not shadows.
- **Hard offset shadows, not soft ones.** If a shadow is used it is a solid black block offset (`box-shadow: 6px 6px 0 #000`), never a blurred tinted shadow. The base skill's "tint the shadow, no pure black" rule is inverted: shadows here are pure black, zero blur, hard offset.
- **No glass, no frost, no backdrop-filter.** Flat, opaque, built.

### 4.0.C Type (inverts 4.1)

- **System and monospace fonts are ENCOURAGED, not banned.** Helvetica / Arial / system-ui set huge, or a monospace (JetBrains Mono, Space Mono, IBM Plex Mono) as a display face. The base skill discourages system fonts and Inter; brutalism reclaims them as a deliberate raw signal.
- **Serif is fine here** when it is a hard editorial slab or a stark Times-like face used at brutal scale. The base skill's serif ban is lifted; brutalism can use serif as confrontation, not decoration.
- **Type as structure.** Oversized, tight-tracked, sometimes ALL CAPS, sometimes underlined like raw HTML. Mixed sizes slammed together. Type is the loudest layout element.
- **Raw-HTML signals are features (visually only):** visible underlines on links, default-blue link color used ironically, monospace labels, exposed field outlines. This is about the raw *look*, NOT about skipping input sanitization. User-supplied content is still sanitized per Section 6.5 - brutalism keeps the raw aesthetic, never a raw XSS hole.

### 4.0.D What still holds (do NOT invert these)

Brutalism is an aesthetic, not an excuse to ship broken or inaccessible work. These base-skill rules stay in full force:

- **Contrast / accessibility (a11y).** Fluorescent-on-black and black-on-fluorescent must still pass WCAG AA. Loud is fine; unreadable is not. The Button Contrast Check still applies. Electric yellow on white fails contrast; do not do it.
- **The em-dash ban.** Still zero em-dashes. Brutalism is about visual rawness, not punctuation.
- **Real images, no fakes.** Still no div-based fake screenshots. Brutalist sites use real images, often with hard duotone or high-contrast black-and-white treatment, or raw uncropped photos. But real.
- **Performance gate (Section 15).** Still measured, still Performance >= 90. Flat color and hard borders are actually cheap to render, so brutalism should score HIGH; there is no excuse to miss the gate.
- **Layout Casting (1.5) and its variety floor.** Still cast a varied structure. Brutalism is not "one long scroll of black boxes"; it still needs structural variety, just rendered brutally.
- **Reduced motion, hero discipline, one-theme lock.** All still apply.

### 4.0.E The failure mode to avoid

The brutalist failure mode is **"looks accidentally broken"**: inconsistent border widths, three random fluorescents that were not chosen on purpose, one rounded corner that slipped through, a soft shadow left in from a default component. Brutalism reads as intentional ONLY when the hard choices are applied with total consistency. Inconsistency is what makes it look like a mistake. Audit for consistency the way the base skill audits for polish: every corner sharp, every border the same black, every accent from the chosen fluorescent set, no soft defaults surviving.
...
* Max 1 accent color. Saturation < 80% by default. **(BRUTALIST OVERRIDE, Section 4.0.A: inverted - fluorescent full-saturation accents, and 2-3 accents allowed if chosen up front. Follow 4.0.)**
...
* When a shadow is used, tint it to the background hue. No pure-black drop shadows on light backgrounds. **(BRUTALIST OVERRIDE, Section 4.0.B: inverted - shadows are hard pure-black offset (`6px 6px 0 #000`), zero blur. Follow 4.0.)**
...
* **No pure `#000000` and no pure `#ffffff`** - use off-black (zinc-950, near-black warm gray) and off-white. Pure values kill depth. **(BRUTALIST OVERRIDE, Section 4.0.A: this is inverted - pure #000/#fff are REQUIRED here. Follow 4.0, ignore this line.)**
...
### 9.A Visual & CSS
* **NO neon / outer glows** by default. Use inner borders or subtle tinted shadows. **(BRUTALIST NOTE: flat fluorescent fills are used here, but the anti-GLOW part still holds - flat color yes, soft outer glow no. See 4.0.A.)**
* **NO pure black (`#000000`).** Off-black, zinc-950, or charcoal. **(BRUTALIST OVERRIDE, Section 4.0.A: inverted - pure black is required. Follow 4.0.)**
...
- [ ] **XSS guardrails (Section 6.5)**: no unsanitized `dangerouslySetInnerHTML`/`innerHTML`; user URLs allow-listed by scheme (no `javascript:`); markdown sanitized; no string-built dynamic script? Raw-HTML *look* is fine, raw XSS hole is not.
