...
- **UX Writing / Anti-AI-Copy (Section 9.5)** - the verbal counterpart to the visual AI-tell bans. Fixes the four structural tells of AI copy (empty adjectives instead of facts, uniform sentence rhythm, feeling-first headlines that state nothing, setup-phrase habit) as a rewrite discipline rather than a word blacklist. Language-agnostic (English and Korean as examples), native-language voice (no translationese), and ties into Pre-Flight. Goes past the base skill, which bans a few cliche words but does not correct copy structure.
